    ...
    <div class="max-w-5xl mx-auto p-4 sm:p-6 space-y-5">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          ...
          <!-- 가져오기·CSV 버튼은 요청에 따라 숨김 처리 (기능·바인딩은 유지, hidden 제거로 복원) -->
          <button id="importCsvBtn" class="hidden px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 transition-colors items-center gap-1.5">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
            가져오기
          </button>
          <button id="exportCsvBtn" class="hidden px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 transition-colors">CSV</button>
