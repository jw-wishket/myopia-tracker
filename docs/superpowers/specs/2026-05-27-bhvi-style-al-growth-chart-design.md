...
```
R = α + β·L                  # β<0
기본값(이미지 정확 매칭): β=-0.9 D/mm, α=21.189 → R=0 인 emmetropic AL ≈ 23.543mm
   검증: R(25.21mm) = 21.189 − 0.9·25.21 = −1.50D (이미지 우안 밴드 중심과 정확 일치) ✓

예측 안축장 분포:  AL ~ N(L̂, σ_AL²),  σ_AL = PREDICTION_SD (기본 1.111mm)
선형변환 → 굴절:   R ~ N(α+β·L̂, (|β|·σ_AL)²)
   검증: σ_R = |β|·σ_AL = 1.000D → 2σ 밴드 −1.50 ± 2·σ_R = (−3.50, +0.50) (이미지 밴드와 정확 일치) ✓
우측 패널 = 종형곡선 + 2σ 밴드(평균 ± 2·σ_R)
   ※ BHVI 레퍼런스는 1.96σ(95%)가 아닌 2σ(≈95.45%) 신뢰구간 사용으로 추정.
```
> **주의 (정합 결합):** β와 σ_AL은 독립이 아니다. 2σ 밴드폭 = 4·|β|·σ_AL 이므로 함께 보정해야
> 한다(|β|·σ_AL = 1.0). 또한 β=−0.9는 "예측 성인 AL → 성인 굴절" **횡단면** 변환 기울기로,
> 개인내 안축장 신장-굴절 기울기(≈−2.3 D/mm)보다 완만하며 문헌 모집단 범위(−1.2~−1.5)의
> 하단이다. 이미지 재현용 보정값이므로 §8에 따라 클리닉 자체 데이터로 재보정 대상.
...
REFRACTION_MODEL = { alpha: 21.189, beta: -0.9, emmetropiaAL: 23.543 };  // R=α+β·AL, 이미지 정확 매칭
PREDICTION_SD    = 1.111;   // mm (|β|·σ_AL = 1.0 → 2σ 밴드 ±2.0D)
RISK_THRESHOLDS  = {
...
2. **AL→굴절 계수(β,α)와 σ_AL** ⚠️: 이미지 우측 패널의 **중심·밴드폭에 맞춰 보정한 근사값**
   이며, β와 σ_AL은 독립이 아니라 함께 보정됨(|β|·σ_AL ≈ 밴드폭/3.92 ≈ 1.02). 채택한 횡단면
   기울기 β=−0.9는 문헌 모집단 범위(−1.2~−1.5)의 하단이다. 프로젝트가 SE를 저장하므로,
   **클리닉 자체 AL–SE 데이터로 회귀 재보정 권장.**
   → 구현 계획에 **"AL→SE 변환 보정 검증" 태스크**를 반드시 포함할 것.
3. **위험도 임계값/가중**: BHVI 정확값 비공개. 임상적으로 타당한 기본값을 채택, 설정으로 조정.
4. **위험도 게이지 시각 단순화 (의도된 차이)**: 이미지 게이지는 두 개의 바(회색 위 / 컬러 아래,
   "12.5세"·"13.4세" 라벨)로 보인다. 본 설계는 이를 **단일 바 + 현재/직전 방문 마커**로 단순화한다.
   의도된 단순화이며 버그 아님.
5. **진행속도 산출 (V1 한계)**: `progressLabel` 방식(최근 두 측정)은 노이즈에 민감해 단일 측정으로
   위험 등급이 흔들릴 수 있다. 기존 패턴과 일관되어 V1 채택, **윈도/회귀 기반 기울기**는 후속 개선.
6. 본 차트는 의사 판단 보조 도구이며 진단을 대체하지 않음.
