...
기본값(이미지 보정): β=-0.9 D/mm, emmetropiaAL=23.5mm → α=21.15
   검증: R(25.21mm) = 21.15 − 0.9·25.21 = −1.54D ≈ 이미지 우안 밴드 중심 ✓

예측 안축장 분포:  AL ~ N(L̂, σ_AL²),  σ_AL = PREDICTION_SD (기본 1.1mm)
선형변환 → 굴절:   R ~ N(α+β·L̂, (|β|·σ_AL)²)
   검증: σ_R = |β|·σ_AL ≈ 1.0D → 95% 밴드 −1.54 ± 1.96·σ_R = (−3.5, +0.5) ≈ 이미지 밴드 ✓
우측 패널 = 종형곡선 + 95% 밴드(평균 ± 1.96·σ_R)
```
> **주의 (정합 결합):** β와 σ_AL은 독립이 아니다. 밴드폭 ≈ 3.92·|β|·σ_AL 이므로 둘을 함께
> 보정해야 한다(|β|·σ_AL ≈ 1.02). 또한 이 β=−0.9는 "예측 성인 AL → 성인 굴절" **횡단면**
> 변환 기울기로, 개인내 안축장 신장-굴절 기울기(≈−2.3 D/mm)보다 완만하며 문헌 모집단
> 범위(−1.2~−1.5)의 하단이다. 이미지 재현용 보정값이므로 §8에 따라 재보정 대상.
...
REFRACTION_MODEL = { alpha: 21.15, beta: -0.9, emmetropiaAL: 23.5 };  // R=α+β·AL, 이미지 보정·재보정 대상
PREDICTION_SD    = 1.1;   // mm (β와 결합 보정: |β|·σ_AL ≈ 1.02)
RISK_THRESHOLDS  = {
